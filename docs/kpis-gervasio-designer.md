...
- Centros asistenciales que envian mas pacientes, calculado sobre flow_coordina (CodSeguroCoo) y stk_cartera (CodPacFichaCubre).
...
- Top centro por periodo: cantidad de actos del centro que mas pacientes envia en cada periodo.
...
- Volumen mensual de actividad total, calculado sobre flow_coordina y por tipo de acto (CodCoordinaReglaMotivo).
...
- Volumen mensual de cirugias e indicacion de cantidad de reintervenciones; las cirugias se identifican por CodCoordinaReglaMotivo (105, 109, 120).
...
- Reintervenciones sobre el acto: de los actos del mes, identifica los que tienen reintervencion buscando f.Cod en ff.CodActo.
...
- Volumen mensual de actos en hemodinamia por tipo, calculado desde flow_coordina (sTecnica = 'He').
...
- Top tipo de acto de hemodinamia por periodo (aproximacion compatible con KPI Designer para la metrica por tipo).
...
- Tiempo de espera entre inicio del tramite y autorizacion para realizar el acto por parte del FNR, calculado desde flow_coordina.
...
- Tiempo que se demora en realizar el acto desde su autorizacion, calculado desde flow_coordina.
...
- Complicaciones postoperatorias seleccionadas, incluyendo stroke, sepsis y falla renal aguda, a partir de pvd_master y pvd_masterqq (Complica 255).
...
- Complicaciones en las cirugias a partir del 2005; en este KPI se enfoca el porcentaje mensual de stroke postoperatorio.
...
- Perfil demografico quirurgico, incluyendo edad promedio, mayores de 75 anos y distribucion por sexo.
